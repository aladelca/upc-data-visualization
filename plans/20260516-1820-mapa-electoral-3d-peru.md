...
- El producto debe mostrar una columna por distrito como vista principal, con color por ganador y altura por margen electoral, e incluir una vista alternativa con distritos extruidos.
- El pipeline de datos debe ser reproducible: descargar o registrar fuentes, agregar resultados ONPE por distrito, unir con geometria distrital INEI, calcular metricas y exportar GeoJSON optimizado para la app.
...
- Para mantener rigor de datos, el frontend no debe agregar votos. La agregacion por ubigeo y el join espacial deben quedar resueltos antes de publicar el GeoJSON.
...
- Agregar tests Python para agregacion electoral, calculo de ganador, margen, normalizacion de alturas y validacion de join por ubigeo.
...
| `scripts/download_election_sources.py` | create | Descargar la data ONPE y, si el servicio lo permite, la geometria distrital. Si no, validar presencia de archivos manuales en `data/raw/`. |
| `scripts/build_election_geojson.py` | create | Leer resultados por mesa, agregar por distrito, calcular metricas electorales, unir con geometria, generar centroides y exportar GeoJSON procesado. |
...
| `apps/peru-election-3d-map/src/scales.ts` | create | Escalas de color y altura para ganador, margen, votos validos y participacion. |
| `apps/peru-election-3d-map/src/types.ts` | create | Tipos TypeScript del GeoJSON procesado y estado de visualizacion. |
| `apps/peru-election-3d-map/src/ui.ts` | create | Controles de capa, metrica de altura, busqueda simple y leyenda. |
...
- Frontend contract:
  ...
  - La app puede cambiar visualmente la metrica de altura, pero no recalcular resultados electorales base.
...
3. Implementar `scripts/election_geo_processing.py` con funciones puras:
   ...
   - `validate_geo_join`
4. Implementar `scripts/download_election_sources.py` para descargar ONPE desde Datos Abiertos y registrar metadatos locales. Para INEI, primero intentar endpoint WFS; si falla o requiere flujo manual, dejar error claro con instrucciones.
5. Implementar `scripts/build_election_geojson.py` como comando reproducible:
   ...
   - calcular centroides en CRS proyectado adecuado antes de volver a WGS84
...
6. Crear tests en `tests/test_election_geo_processing.py` con data sintetica para:
   ...
   - altura capada para evitar que Lima aplaste la escala visual
...
9. Implementar `ColumnLayer` como vista principal:
   ...
   - color: ganador
   - elevacion: `height_margin` por defecto
10. Implementar `GeoJsonLayer` extruido como vista alternativa:
    - color: ganador
    - elevacion: margen o votos validos logaritmicos
    - bordes sutiles para legibilidad
11. Implementar UI:
    - selector de capa: columnas / distritos extruidos
    - selector de altura: margen porcentual / votos validos log / margen absoluto
...
- Unit: `tests/test_election_geo_processing.py` cubrir agregacion por distrito, normalizacion de ubigeo, ganador, margen y alturas.
...
- Data build: `python scripts/build_election_geojson.py --onpe data/raw/onpe_segunda_vuelta_2021.zip --districts data/raw/inei_distritos.geojson --out data/processed`
...
- Riesgo: altura por votos produce dominancia visual de Lima. -> Mitigar usando margen porcentual como default y votos validos en escala logaritmica como opcion.
...
- El color identifica correctamente el ganador distrital entre Peru Libre y Fuerza Popular.
- La altura default representa margen porcentual y evita que distritos muy poblados dominen el grafico.
...
- Al probar ONPE + WFS INEI, el join directo por `ubigeo` solo empato `1102` de `1890` geometrias. Se agrego fallback conservador por `departamento + distrito` cuando el nombre es unico dentro del departamento. Con eso el build real empato `1844` de `1890` geometrias: `1102` por ubigeo y `742` por nombre.
